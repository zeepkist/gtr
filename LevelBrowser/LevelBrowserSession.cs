...
/// set of already-in level UIDs, and the discovery UI state (search text, page). It knows nothing
/// about playlists, Unity, Imui, or GraphQL — a host opens it, receives one
...
public sealed class LevelBrowserSession
{
    ...
    /// already-in set and resets discovery UI state (search text, page). Opening never notifies a
    /// previous host of a close — a rebind is not a user close.
    /// </summary>
